...
class Life():
    ...
    def click(self, surface):
        if self.alive is False:
            self.setAlive(surface)

        elif self.alive is True:
            self.setDead(surface)
    # Feito para loop de verificação

    def setAlive(self, surface):
        surface[self.num[0]][self.num[1]].fill(ALIVE_COLOR)
        self.alive = True

    def setDead(self, surface):
        surface[self.num[0]][self.num[1]].fill(BACKGROUND_COLOR)
        self.alive = False
        ...
        for line in range(-1, 2):
            ...
            for cell in range(-1, 2):
                ...
                if num[1]+cell == -1:
                    ...
                    c_cell = num[1]+cell
                ver = sprites[c_line][c_cell].alive
                if ver is True:
                    nb += 1
            self.nb = nb
...
def main():
    ...
    active_box = background.subsurface((ACTIVE_BOX_POS, ACTIVE_BOX_SIZE))
    ...
    iterator_box = background.subsurface(ITERATOR_BOX_POS, ITERATOR_BOX_SIZE)
    # Iterator Number
    iterator_box.blit(pygame.font.SysFont(FONT_USED, COUNTER_FONT_SIZE).render
                      (str(num_of_cycles), False, COUNTER_FONT_COLOR),
                      COUNTER_FONT_POS)
    # speed of iteration
    speed_box = background.subsurface(SPEED_BOX_POS, SPEED_BOX_SIZE)
    speed_box.blit(pygame.font.SysFont(FONT_USED, SPEED_FONT_SIZE).render
                   ('1FPS', False, (250, 250, 250)), SPEED_FONT_POS)
    ...
    for line in range(AMOUNT_OF_LINES):
        for unid in range(AMOUNT_OF_CELLS):
            ...
            iline.append(life)
            sur = background.subsurface(life.rect)
            sline.append(sur)
            if px <= PLAY_SCREEN_WIDTH:
                px += BOX_SIZE
        else:
            indexBox.append(iline)
            surfaceBox.append(sline)
            sline = []
            iline = []
        px = 1
        py += BOX_SIZE
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            # Código para mudar se cada célula está viva ou morta
                ...
                if activeSim is False:
                    # After getting mouse pos...
                    pos = pygame.mouse.get_pos()
                    apx, apy = pos
                    # Then is made incrementes for new positions
                    apx = apx // BOX_SIZE
                    apy = apy // BOX_SIZE
                # Pegando a célula da lista, e mandando ela atualizar
                print(indexBox[apy][apx].alive)
                indexBox[apy][apx].click(surfaceBox)
            ...
            # Lendo o input do teclado
                ...
                if event.key == pygame.K_l:
                    for line in indexBox:
                        for cell in line:
                            cell.setDead(surfaceBox)
                    file = open("GameSave.txt", 'r')
                    for position in file:
                        load_line = int(position[1:3])
                        load_cell = int(position[5:7])
                        indexBox[load_line][load_cell].setAlive(surfaceBox)
                    file.close()
                ...
                if event.key == pygame.K_SPACE:
                    if activeSim is False:
                        ...
                        active_box.blit(pygame.font.SysFont(
                                        FONT_USED, FONT_SIZE).
                                        render('Active', False, FONT_COLOR),
                                        ACTIVE_TEXT_POS)
                        ...
                        active_box.blit(pygame.font.SysFont(
                                        FONT_USED, FONT_SIZE).
                                        render('Inactive', False, FONT_COLOR),
                                        INACTIVE_TEXT_POS)
                ...
                if event.key == pygame.K_1:
                    ...
                    speed_box.blit(pygame.font.SysFont(FONT_USED,
                                   SPEED_FONT_SIZE).render
                                   ('1FPS', False, SPEED_FONT_COLOR),
                                   SPEED_FONT_POS)
        ...
        if activeSim:
            ...
            for line in indexBox:
                for cell in line:
                    cell.commit(surfaceBox)
            iterator_box.fill(BACKGROUND_COLOR)
            num_of_cycles += 1
            iterator_box.blit(pygame.font.SysFont(FONT_USED,
                              COUNTER_FONT_SIZE).render
                              (str(num_of_cycles), False,
                              COUNTER_FONT_COLOR), COUNTER_FONT_POS)
